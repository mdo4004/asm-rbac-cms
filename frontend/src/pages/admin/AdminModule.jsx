...
const fmt = (v) => {
  ...
  if (typeof v === 'string' && /^\d{4}-\d{2}-\d{2}/.test(v)) {
    try { const d = new Date(v); if (!isNaN(d)) return d.toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'}); } catch(e){}
  }
  return String(v);
};
...
function countActive(vals) {
  return Object.values(vals).filter(v => v && v !== '').length;
}

// ── MASTER DATA CATEGORIES ────────────────────────────────────
const MASTER_TABS = [
  { key:'firms',      label:'Selling Firm',  icon:'🏢', fields:[ { key:'name', label:'Firm Name', required:true }, { key:'prefix', label:'CPO Prefix' }, { key:'phone', label:'Phone' }, { key:'city', label:'City' } ]},
  { key:'qualities',  label:'Quality',       icon:'🧵', fields:[ { key:'name', label:'Quality Name', required:true }, { key:'shrinkage_pct', label:'Shrinkage %', type:'number' }, { key:'description', label:'Description', type:'textarea' } ]},
  { key:'customers',  label:'Customer',      icon:'👤', fields:[ { key:'name', label:'Customer Name', required:true }, { key:'credit_days', label:'Credit Days', type:'number' }, { key:'phone', label:'Phone' }, { key:'city', label:'City' } ]},
  { key:'followups',  label:'Followuper',    icon:'📞', fields:[ { key:'name', label:'Name', required:true }, { key:'phone', label:'Phone' }, { key:'department', label:'Department' } ]},
  { key:'mills',      label:'Mill Name',     icon:'🏭', fields:[ { key:'name', label:'Mill Name', required:true }, { key:'phone', label:'Phone' }, { key:'city', label:'City' } ]},
  { key:'merchants',  label:'Merchant',      icon:'💼', fields:[ { key:'name', label:'Merchant Name', required:true }, { key:'phone', label:'Phone' } ]},
  { key:'statuses',   label:'Status',        icon:'🏷️', fields:[ { key:'name', label:'Status Name', required:true } ]},
  { key:'suppliers',  label:'Supplier',      icon:'📦', fields:[ { key:'name', label:'Supplier Name', required:true }, { key:'phone', label:'Phone' }, { key:'city', label:'City' } ]},
  { key:'work_types', label:'Work Type',     icon:'⚒️', fields:[ { key:'name', label:'Work Type', required:true } ]},
  { key:'buyers',     label:'Buyer',         icon:'🛒', fields:[ { key:'name', label:'Buyer Name', required:true }, { key:'phone', label:'Phone' }, { key:'city', label:'City' } ]},
];

// ── Reusable Components ───────────────────────────────────────
function StatusBadge({ val }) {
  const map = { completed:'bg-green-100 text-green-700', approved:'bg-green-100 text-green-700', sent:'bg-green-100 text-green-700', converted:'bg-green-100 text-green-700', cancelled:'bg-red-100 text-red-600', rejected:'bg-red-100 text-red-600', in_progress:'bg-blue-100 text-blue-700', follow_up:'bg-blue-100 text-blue-700', pending:'bg-yellow-100 text-yellow-700', new:'bg-yellow-100 text-yellow-700', processed:'bg-gray-100 text-gray-600', closed:'bg-gray-100 text-gray-600' };
  return <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${map[val]||'bg-gray-100 text-gray-600'}`}>{val||'—'}</span>;
}
...
function Pagination({ page, total, perPage, setPage }) {
  ...
  return (
    <div className="flex items-center justify-between px-5 py-3 border-t text-sm text-gray-500 bg-gray-50">
      <span className="text-xs">Showing {(page-1)*perPage+1}–{Math.min(page*perPage,total)} of <strong>{total}</strong></span>
      <div className="flex gap-1">
        <button disabled={page===1} onClick={()=>setPage(p=>p-1)} className="px-3 py-1.5 rounded-lg border bg-white disabled:opacity-40 hover:bg-gray-100 text-xs font-medium">← Prev</button>
        {nums.map(p=><button key={p} onClick={()=>setPage(p)} className={`px-3 py-1.5 rounded-lg border text-xs font-medium ${page===p?'bg-blue-600 text-white border-blue-600':'bg-white hover:bg-gray-100'}`}>{p}</button>)}
        <button disabled={page===pages} onClick={()=>setPage(p=>p+1)} className="px-3 py-1.5 rounded-lg border bg-white disabled:opacity-40 hover:bg-gray-100 text-xs font-medium">Next →</button>
      </div>
    </div>
  );
}
...
function MasterDataModule() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      if (editRow) { await api.put(`/master/${activeTab}/${editRow.id}`, form); toast.success('Updated!'); }
      else { await api.post(`/master/${activeTab}`, form); toast.success('Added!'); }
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-black text-gray-800">📋 Master Data</h1>
          <p className="text-gray-400 text-sm mt-0.5">Manage master records used in all form dropdowns</p>
        </div>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
        {MASTER_TABS.map(t => (
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-semibold border transition-all ${activeTab===t.key?'bg-blue-600 text-white border-blue-600 shadow-md':'bg-white text-gray-600 border-gray-200 hover:border-blue-300 hover:text-blue-600'}`}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b bg-gray-50">
          ...
          <div className="flex gap-2">
            ...
            {canAdd && (
              <button onClick={() => { setEditRow(null); setForm({}); setShowForm(true); }}
        ...
        {showForm && (
          <div className="border-b bg-blue-50 p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-blue-800">{editRow ? `✏️ Edit ${tab.label}` : `➕ New ${tab.label}`}</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {tab.fields.map(f => (
                <div key={f.key} className={f.type==='textarea'?'col-span-full':''}>
                  <label className="form-label">{f.label}{f.required&&<span className="text-red-500 ml-1">*</span>}</label>
                  {f.type==='textarea' ? <textarea className="form-input" rows={2} value={form[f.key]||''} onChange={e=>setForm(p=>({...p,[f.key]:e.target.value}))}/> :
                   f.type==='number' ? <input type="number" className="form-input" value={form[f.key]||''} onChange={e=>setForm(p=>({...p,[f.key]:e.target.value}))}/> :
                   <input type="text" className="form-input" value={form[f.key]||''} placeholder={f.placeholder||f.label} onChange={e=>setForm(p=>({...p,[f.key]:e.target.value}))}/>}
                </div>
              ))}
            </div>
            <div className="flex gap-3 mt-4">
              <button onClick={handleSave} disabled={saving} className="bg-blue-600 text-white px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-blue-700 disabled:opacity-50 transition">
                {saving?'Saving…':editRow?'Update':'Save'}
              </button>
              <button onClick={()=>setShowForm(false)} className="px-6 py-2.5 rounded-xl text-sm font-bold border border-gray-200 hover:bg-gray-100 transition">Cancel</button>
            </div>
          </div>
        )}
          ...
          <table className="w-full">
            <thead>
              <tr>
                ...
                {tab.fields.slice(0,4).map(f=><th key={f.key} className="table-th">{f.label.toUpperCase()}</th>)}
                <th className="table-th text-right">ACTIONS</th>
              </tr>
            </thead>
            <tbody>
              {loading && <tr><td colSpan={tab.fields.slice(0,4).length+2} className="table-td text-center py-12 text-gray-400"><div className="flex flex-col items-center gap-2"><div className="w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"/><span>Loading…</span></div></td></tr>}
              {!loading && !rows.length && <tr><td colSpan={tab.fields.slice(0,4).length+2} className="table-td text-center py-12 text-gray-400"><div className="text-3xl mb-2">📂</div><p className="font-semibold">No {tab.label} records yet</p></td></tr>}
              {rows.map((row,i) => (
                <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                  <td className="table-td text-gray-400 text-xs font-mono">{(page-1)*PER_PAGE+i+1}</td>
                  {tab.fields.slice(0,4).map(f=><td key={f.key} className="table-td max-w-[160px] truncate" title={fmt(row[f.key])}>{fmt(row[f.key])}</td>)}
                  <td className="table-td">
                    <div className="flex items-center justify-end gap-1">
                      {canEdit && <button onClick={()=>{setEditRow(row);setForm({...row});setShowForm(true);}} className="text-xs text-blue-600 font-semibold px-2.5 py-1.5 rounded-lg hover:bg-blue-50 border border-blue-200 whitespace-nowrap">✏️ Edit</button>}
...
function SmartModule({ cfg, apiPath }) {
  ...
  const filterCfg = FILTER_CONFIG[apiPath] || [];
  ...
  const modKey    = apiPath === 'customer-po' ? 'customer_po' : apiPath;
  ...
  const load = useCallback(() => {
    setLoading(true);
    const fp = filtersToParams(appliedFilters);
    api.get(`/${apiPath}`, { params: { page, limit:PER_PAGE, search, ...fp } })
      .then(r => { setRows(r.data.data||[]); setTotal(r.data.total||0); })
      .catch(()=>{}).finally(()=>setLoading(false));
  ...
  const openCreate = () => { setEditRow(null); setForm(defaultForm()); setShowForm(true); setCpoPreview(''); cpoFetched.current=false; };
  const openEdit   = (row) => { setEditRow(row); setForm({...row}); setShowForm(true); setCpoPreview(row.cpo_number||''); };
  ...
  useEffect(() => {
    ...
    if (apiPath==='inward' && form.rate && form.grey_meter)
      setForm(p=>({...p,amount:String((parseFloat(p.rate)*parseFloat(p.grey_meter)*1.05).toFixed(2))}));
    if (apiPath==='jobwork' && form.rate && form.finish_meter)
      setForm(p=>({...p,amount:String((parseFloat(p.rate)*parseFloat(p.finish_meter)).toFixed(2))}));
  }, [form.rate, form.grey_meter, form.finish_meter, apiPath]);

  // CPO number preview
  useEffect(() => {
    if (apiPath!=='customer-po' || editRow || !form.selling_firm) { setCpoPreview(''); return; }
    const firm = (masters.firms||[]).find(f=>f.name===form.selling_firm);
    if (!firm?.prefix) return;
    api.get(`/customer-po/preview-no?prefix=${firm.prefix}`).then(r=>{ if(r.data.success) setCpoPreview(r.data.cpo_number); }).catch(()=>{});
  }, [form.selling_firm, apiPath, editRow, masters.firms]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editRow) { await api.put(`/${apiPath}/${editRow.id}`, payload); toast.success(`${cfg.title} updated!`); }
      else { const res=await api.post(`/${apiPath}`, payload); toast.success(`${cfg.title} saved! ${res.data.cpo_number?`CPO: ${res.data.cpo_number}`:''}`); }
      handleCancel(); load();
  ...
  const renderField = (fld) => {
    ...
    if (fld.type==='cpo') return (
      <div className="form-input bg-gray-100 text-gray-600 font-mono text-sm flex items-center gap-2">
        <span className="text-blue-500">🔢</span>
        <span className="font-bold text-blue-700">{editRow?(form.cpo_number||'Will be assigned'):(cpoPreview||'Select Selling Firm first')}</span>
      </div>
    );
    if (fld.type==='select-master') {
      const opts = fld.master==='firms' ? firmNames : (masters[fld.master]||[]);
      return <select className={cls} value={val} disabled={fld.readOnly} onChange={e=>setField(fld.key,e.target.value)}><option value="">— Select —</option>{opts.map(o=><option key={o} value={o}>{o}</option>)}</select>;
    }
    if (fld.type==='combobox-master') return <Combobox value={val} onChange={v=>setField(fld.key,v)} options={masters[fld.master]||[]} placeholder="Select or type quality..."/>;
    if (fld.type==='select') return <select className={cls} value={val} onChange={e=>setField(fld.key,e.target.value)}>{(fld.options||[]).map(o=><option key={o} value={o}>{o||'— Select —'}</option>)}</select>;
    if (fld.type==='date') return <input type="date" className={cls} value={val} readOnly={fld.readOnly} onChange={e=>setField(fld.key,e.target.value)}/>;
    if (fld.type==='textarea') return <textarea className={cls} rows={3} value={val} placeholder={fld.placeholder||''} onChange={e=>setField(fld.key,e.target.value)}/>;
    if (fld.type==='number') return <input type="number" className={cls} value={val} readOnly={fld.readOnly} placeholder={fld.placeholder||''} onChange={e=>{ if(fld.key==='amount') setForm(p=>({...p,amount:e.target.value,_amountManual:true})); else setField(fld.key,e.target.value); }}/>;
    return <input type="text" className={cls} value={val} readOnly={fld.readOnly} placeholder={fld.placeholder||''} onChange={e=>setField(fld.key,e.target.value)}/>;
  };
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-gray-800">{cfg.title} <span className="text-gray-400 text-lg font-normal">{cfg.subtitle}</span></h1>
            <p className="text-gray-400 text-sm">{total} total records</p>
          </div>
        </div>
        {canAdd && !showForm && (
          <button onClick={openCreate} className={`text-white px-5 py-2.5 rounded-xl text-sm font-bold transition ${color.btn}`}>
            + Add New {cfg.title}
          </button>
        )}
      </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <div className={`flex items-center justify-between px-6 py-4 bg-gradient-to-r ${color.header}`}>
            <h2 className="font-bold text-white text-lg">
              {editRow ? `✏️ Edit ${cfg.title}` : `➕ New ${cfg.title}`}
            </h2>
            <button onClick={handleCancel} className="text-white/80 hover:text-white text-2xl font-light">✕</button>
          </div>
          <div className="p-6 grid grid-cols-2 gap-4">
            {cfg.fields.map(fld => (
              <div key={fld.key} className={fld.span===2?'col-span-2':''}>
                <label className="form-label">
                  {fld.label}{fld.required&&<span className="text-red-500 ml-1">*</span>}
                  {fld.autoNote&&<span className="text-gray-400 font-normal ml-1">({fld.autoNote})</span>}
                </label>
                {renderField(fld)}
              </div>
            ))}
          ...
          <div className="px-6 pb-6 flex gap-3">
            <button onClick={handleSave} disabled={saving} className={`flex-1 text-white py-3 rounded-xl text-sm font-bold transition-all disabled:opacity-50 ${color.btn}`}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b bg-gray-50/50">
          <div className="flex items-center gap-2">
            <h3 className="font-bold text-gray-700 text-sm">All Records</h3>
            {activeCount > 0 && (
              <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-semibold">
                {activeCount} {activeCount > 1 ? 'filters' : 'filter'} active
              </span>
            )}
          </div>
        ...
        {filterCfg.length > 0 && (
          <div className="px-5 pt-4">
            <FilterPanel config={filterCfg} values={filterVals} onChange={handleFilterChange}
              onApply={handleFilterApply} onReset={handleFilterReset} activeCount={activeCount} />
          </div>
        )}
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr>
                <th className="table-th w-10">#</th>
                {cfg.tableFields.map(c=><th key={c} className="table-th">{c.replace(/_/g,' ').replace(/\b\w/g,l=>l.toUpperCase())}</th>)}
                <th className="table-th text-right">ACTIONS</th>
              </tr>
            </thead>
            <tbody>
              {loading && <tr><td colSpan={cfg.tableFields.length+2} className="table-td text-center py-12 text-gray-400"><div className="flex flex-col items-center gap-2"><div className="w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"/><span>Loading…</span></div></td></tr>}
              {!loading && !rows.length && <tr><td colSpan={cfg.tableFields.length+2} className="table-td text-center py-16 text-gray-400"><div className="text-4xl mb-2">📋</div><p className="font-semibold">No records found</p></td></tr>}
              {rows.map((row,i)=>(
                <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                  <td className="table-td text-gray-400 text-xs font-mono">{(page-1)*PER_PAGE+i+1}</td>
                  {cfg.tableFields.map(c=>(
                    <td key={c} className="table-td max-w-[140px] truncate" title={fmt(row[c])}>
...
// ── GENERIC MODULE (Enquiry / Returns / Sampling) ─────────────
function GenericModule({ title, apiPath }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setShowForm(false); setEditRow(null); setForm({}); load();
    } catch(e) { toast.error(e.response?.data?.message||'Save failed'); }
    finally { setSaving(false); }
  };
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-2 items-center">
          <input className="form-input w-44" placeholder="🔍 Search…" value={search}
            onChange={e => { setSearch(e.target.value); setPage(1); }} />
          {canAdd && (
            <button onClick={()=>{setEditRow(null);setForm({});setShowForm(true);}} className="btn-primary">
              + Add New
            </button>
          )}
        </div>
      ...
      {activeCount > 0 && (
        <div className="flex items-center gap-2 text-sm text-blue-700 bg-blue-50 px-4 py-2 rounded-lg border border-blue-200">
          <span>🔍</span>
          <span>{activeCount} {activeCount>1?'filters':'filter'} active —</span>
          <button onClick={handleFilterReset} className="underline font-semibold">Clear all</button>
        </div>
      )}
          ...
          <div className="p-6 grid grid-cols-2 gap-4">
            {cols.map(c => (
              <div key={c} className={(c.includes('notes')||c.includes('reason')||c.includes('requirement'))?'col-span-2':''}>
                ...
                ) : c.includes('qty')||c.includes('meter')||c.includes('amount')||c.includes('pieces')||c==='rate'||c==='quantity'||c==='meters' ? (
                  <input type="number" className="form-input" value={form[c]||''} onChange={e=>setForm(p=>({...p,[c]:e.target.value}))}/>
                ) : (
                  <input type="text" className="form-input" value={form[c]||''} onChange={e=>setForm(p=>({...p,[c]:e.target.value}))}/>
                )}
              </div>
            ))}
          </div>
          <div className="px-6 pb-6 flex gap-3">
            <button onClick={handleSave} disabled={saving} className="flex-1 bg-blue-600 text-white py-3 rounded-xl text-sm font-bold hover:bg-blue-700 disabled:opacity-50">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr>
                <th className="table-th w-10">#</th>
                {cols.map(c=><th key={c} className="table-th">{c.replace(/_/g,' ').replace(/\b\w/g,l=>l.toUpperCase())}</th>)}
                <th className="table-th text-right">ACTIONS</th>
              </tr>
            </thead>
            <tbody>
              {loading && <tr><td colSpan={cols.length+2} className="table-td text-center py-12"><div className="flex flex-col items-center gap-2 text-gray-400"><div className="w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"/><span>Loading…</span></div></td></tr>}
              {!loading && !rows.length && <tr><td colSpan={cols.length+2} className="table-td text-center py-12 text-gray-400"><div className="text-3xl mb-2">📋</div><p>No records found</p></td></tr>}
              {rows.map((row,i)=>(
                <tr key={row.id} className="hover:bg-gray-50">
                  <td className="table-td text-gray-400 text-xs font-mono">{(page-1)*PER_PAGE+i+1}</td>
                  {cols.map(c=><td key={c} className="table-td max-w-[130px] truncate" title={fmt(row[c])}>{c==='status'?<StatusBadge val={row[c]}/>:fmt(row[c])}</td>)}
                  <td className="table-td text-right">
                    <div className="flex items-center justify-end gap-1">
                      {canEdit && <button onClick={()=>openEdit(row)} className="text-xs text-blue-600 font-semibold px-2.5 py-1.5 rounded-lg hover:bg-blue-50 border border-blue-200 whitespace-nowrap">✏️ Edit</button>}
                      {canDelete && <button onClick={()=>handleDelete(row.id)} className="text-xs text-red-500 font-semibold px-2.5 py-1.5 rounded-lg hover:bg-red-50 border border-red-200 whitespace-nowrap">🗑️ Delete</button>}
...
// ── MAIN EXPORT ───────────────────────────────────────────────
export default function AdminModule({ title, apiPath, isMaster }) {
